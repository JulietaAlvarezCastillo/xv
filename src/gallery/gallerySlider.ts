...
class GallerySlider {
	...
	private init(): void {
		...
		const container = this.createElement(
			"div",
			{ className: "sectionButtonContainer control d-flex flex-column gap-1" },
			this.container
		);
		...
		for (let index = 0; index < this.sections.length; index++) {
			...
			this.setStateClases(
				this.pagesPerSection[this.currentSection] as HTMLCollection,
				"page"
			);
			...
			if (this.pagesPerSection[index].length > 1) {
				const pageButtonContainer = this.createElement(
					...
					this.sections[index] as HTMLElement
				);
				const pages = this.pagesPerSection[index];

				for (let i = 0; i < pages.length; i++) {
					let pageSpan = parseInt(
						(pages[i] as HTMLElement).dataset.pageSpan || "1",
						10
					);
					if (isNaN(pageSpan)) pageSpan = 1;
					if (pageSpan > 1) {
						(pages[i] as HTMLElement).style.setProperty(
							"--width-multiplier",
							pageSpan.toString()
						);
						...
						this.pagesPerSection[index] = (
							this.sections[index] as HTMLElement
						).getElementsByClassName(this.pageClass);
					}
				}
	...
	private setStateClases(elements: HTMLCollection, type: string): void {
		for (let index = 0; index < elements.length; index++) {
			elements[index].classList.remove(
				`${type}-past`,
				`${type}-future`,
				`${type}-current`
			);
	...
	private addSectionNavigation(index: number, container: HTMLElement): void {
		...
		const label = this.createElement(
			"label",
			{ htmlFor: _id, className: "btn btn-secondary" },
			container
		);
		(label as HTMLElement).dataset.title =
			(this.sections[index] as HTMLElement).dataset.title ?? "";
		(label as HTMLElement).dataset.tooltip = "right";
		(label as HTMLElement).dataset.html = "true";
	}

	private addPageNavigation(
		index: number,
		pageIndex: number,
		...
		try {
			const parentObject =
				typeof parent === "object"
					? parent
					: (document.getElementById(parent as string) as HTMLElement);
	...
	private switchAndTranslate(
		_set: { type: string; page: number } | boolean = false
	): void {
		...
		if (!this.swipeDirection && _set) {
			...
			if ((_set as any).type === "page") {
				const index =
					(_set as any).page - this.currentPage[this.currentSection];
				this.currentPage[this.currentSection] = (_set as any).page;
				this.translate.page[this.currentSection] =
					this.translate.page[this.currentSection] - this.width * index;
				this.pageTranslate();
			}
		}
		...
		// section-${ index }
		const sectionRadio = document.getElementById(
	...
	private sectionTranslate(): void {
		...
		for (let index = 0; index < this.sections.length; index++) {
			(
				this.sections[index] as HTMLElement
			).style.transform = `translateY(${this.translate.section}vh)`;
		}
	}
	...
	private pageTranslate(): void {
		...
		for (
			...
			(
				this.pagesPerSection[this.currentSection][index] as HTMLElement
	...
	private setupEventListeners(): void {
		...
		// Wheel events
		window.addEventListener("wheel", this.swipeWithWheel.bind(this), {
			passive: false,
		});
	...
	private touchStart(event: MouseEvent | TouchEvent): void {
		...
		const avoid = void_target
			? (void_target as HTMLElement).dataset.avoidScroll
			: void_target;
		if (avoid) return;
	...
	private touchEnd(event: MouseEvent | TouchEvent): void {
		...
		const target =
			"changedTouches" in event ? touchEvent.changedTouches[0] : mouseEvent;
	...
	private navButton_manager(): void {
		if (
			this.pagesPerSection[this.currentSection].length ===
